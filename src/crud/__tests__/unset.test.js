...
describe('unset', () => {
  ...
  it('emits storage event when entire namespace is removed', () => {
    const mockEmitStorageEvent = jest.spyOn(window, 'dispatchEvent')
    localStorage.setItem(namespace, JSON.stringify({ key: 'value' }))

    unset(namespace)()

    expect(mockEmitStorageEvent).toHaveBeenCalledTimes(1)
    expect(mockEmitStorageEvent.mock.calls[0][0].type).toBe('storage')
    mockEmitStorageEvent.mockRestore()
  })
})
